...
export default function SSSContribution() {
  ...
  useEffect(() => {
    const fetchSSSDeductionData = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await getSSSDeductionList(vesselId, month, year);
        ...
        console.error("Error fetching SSS deduction data:", error);
        setVesselInfo(null);
        setSSSDeductionData([]);
      } finally {
        setIsLoading(false);
  ...
  const columns: ColumnDef<SSSDeductionCrew>[] = [
    {
        ...
        <div className="rounded-md border pb-3">
          {isLoading ? (
            <div className="flex justify-center items-center p-10">
              ...
              <p>Loading SSS deduction data...</p>
            </div>
          ) : filteredData.length > 0 ? (
            <DataTable columns={columns} data={filteredData} pageSize={6} />
          ) : (
            <div className="flex justify-center items-center p-10">
              <p>No SSS data available for this vessel</p>
            </div>
          )}
        </div>
      </div>
